class Parent < ApplicationRecord
    has_many :students
      ...
      has_many :accounts, as: :accountable
        ...
        validates :code, presence: {:message => "لايجب أن يكون فارغا"}
        validates :code, numericality: { only_integer: true }   
        validates :code, :uniqueness => {:message => "هذا الرقم مستخدم بالفعل"}
        validates :code, length: { maximum: 5, too_long: " يجب أن لا يزيد عن 5 من ( الأرقام ) " }
        validates :code, length: { minimum: 5, too_short: " يجب أن لا يقل عن 5 من ( الأرقام )" }
        ...
        validates :gender, format: { with: /\A[0-9a-zA-Z]*\z/, message: "مسموح باﻷحرف والأرقام فقط" }
        validates :gender, length: { in: 1..10, message: "The number of characters is not valid"  }

        validates :religion, presence: {:message => "لايجب أن يكون فارغا"}
        validates :religion, format: { with: /\A[0-9a-zA-Z]*\z/, message: "مسموح باﻷحرف والأرقام فقط" }
        validates :religion, length: { in: 1..10, message: "The number of characters is not valid"  }
        ...
        validates :national_id, numericality: { only_integer: true } 
        validates :national_id, length: {minimum: 16, too_short: " يجب أن لا يقل عن 16 من [ الأحرف ]"}
        validates :national_id, length: {maximum: 16, too_long: " يجب أن لا يزيد عن 16 من [ الأحرف ]"}
        validates :national_id, presence: {:message => "لايجب أن يكون فارغا"}

        validates :job, presence: {:message => "لايجب أن يكون فارغا"}
        validates :job, format:{ with: /\A[a-zA-Z0-9_]*[a-zA-Z][a-zA-Z0-9]*\z/, message: "مسموح باﻷحرف والأرقام فقط" }
